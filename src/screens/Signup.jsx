...
const Signup = () => {
  ...
  const handleSignup = (e) => {
    ...
    if (currentState === undefined || currentState === null) {
      let currentUsers = [
        { userId: 1, name: "Roshan Horo", username: "_dizit", password: "Roshan@123", profilePic: "https://pbs.twimg.com/profile_images/1284167924123500545/5cloQkyv_400x400.jpg" },
      ...
      currentUsers.push({ userId: lastUser.userId + 1, name, username, password })

      let updatedState = {
        ...currentState, allUsers: currentUsers
      }
    ...
    <div className="login-container" id="signupContainer">
      <div className="sign-up-box">
        <div className="close-div">
          <div id="close-signup-btn" onClick={() => navigate('/')}><FontAwesomeIcon icon={faXmark} size="lg" /></div>
        </div>
        <div className="create-account">
          ...
          {
            nameError === 'success' || nameError === 'init' ? null : <div className="error-text"><span>Name</span> should not be blank and atleast <span>3</span> characters.</div>
          }
          {userRegistered === 'success' || userRegistered === 'init'
            ? null
            : <div className="error-text">Username must be <span>alphanumeric</span> and min <span>5</span> characters.</div>
          }

          {pwdError === 'success' || pwdError === 'init'
            ? null
            : <div className="error-text">password characters not fullfilled, must have one <span>Uppercase</span> and <span>lowercase</span> letter, <span>number</span>, <span>special character</span> and min <span>8</span> character.</div>
          }


          {userRegistered === 'success' && pwdError === 'success' && nameError === 'success' && <div className="login-btn sign-up-create-btn" onClick={handleSignup}>Sign up</div>}
